...
export function NuevaPersonaForm({ onCreada }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-lg border border-[var(--border)] p-4">
      {/* Los tres campos en fila en escritorio: apilados dejaban tres inputs
          de 768px de ancho para textos de dos palabras. */}
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3 md:gap-4">
        <label className="flex flex-col gap-1 text-sm">
          <span className="opacity-70">Nombre</span>
          <input
            className="toque-11 rounded border border-[var(--border)] bg-transparent px-3"
            value={nombre}
